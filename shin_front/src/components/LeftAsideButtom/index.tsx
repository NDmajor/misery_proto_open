...
const LeftAsideButton = memo(function LeftAsideButton(props: IProps) {
  const Data = useMemo(() => {
    switch (props.type) {
      case 'drive':
        return { text: '내 계약', Icon: DriveIcon }

      case 'computers':
        return { text: '컴퓨터', Icon: ComputersIcon }

      case 'shared':
        return { text: '공유 계약', Icon: GroupIcon }

      case 'recent':
        return { text: '최근 계약', Icon: ClockIcon }

      case 'starred':
        return { text: '중요 계약', Icon: StarIcon }

      case 'trash':
        return { text: '휴지통', Icon: TrashIcon }

      case 'storage':
        return { text: '저장 용량', Icon: CloudIcon }
    }
  }, [props.type])
